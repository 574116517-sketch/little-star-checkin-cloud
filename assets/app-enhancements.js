...
(() => {
  ...
  E.parentAdjustments = () => E.mergeAdjustments(...(s.weekData || []).map(week => week?.adjustments || []), s.adjustments || []).filter(item => item.reason !== '现金兑换扣除');
  const childAdjustModal = document.createElement('div'); childAdjustModal.className = 'parent-modal'; childAdjustModal.hidden = true;
  childAdjustModal.innerHTML = '<div class="modal-card"><h2>家长奖惩详情</h2><p>这里保留爸爸妈妈历次奖励与惩罚记录。</p><div id="eChildAdjustList" class="stats-list"></div><div class="modal-actions"><button class="primary" onclick="closeChildAdjustmentDetail()">我知道了</button></div></div>';
  ...
  E.renderAdjustmentCard = () => { const card = $('.extra-card'), isParent = roleName !== '孩子', records = E.parentAdjustments(), recent = records.slice(0, 3), plus = records.reduce((sum, item) => sum + Math.max(0, Number(item.n || 0)), 0), minus = records.reduce((sum, item) => sum + Math.abs(Math.min(0, Number(item.n || 0))), 0); card.classList.add('child-adjust-card'); card.innerHTML = `<div><div class="adjust-head"><h2>家长奖惩详情</h2><span class="adjust-summary"><b class="plus">+${plus}</b><b class="minus">-${minus}</b></span></div><p class="extra-help">${isParent ? '历次家长奖励、惩罚会保留在这里；调整本周积分不会进入此处。' : '爸爸妈妈给你的鼓励和提醒，都会记录在这里。'}</p><div class="stats-list">${recent.length ? recent.map(E.adjustmentRow).join('') : '<div><span>暂时还没有新的记录</span><b>—</b></div>'}</div></div><div class="parent-adjust-actions"><button class="softbtn" onclick="openChildAdjustmentDetail()">查看详情</button>${isParent ? '<button class="softbtn" onclick="openPointControl()">调整积分</button>' : ''}</div>`; };
